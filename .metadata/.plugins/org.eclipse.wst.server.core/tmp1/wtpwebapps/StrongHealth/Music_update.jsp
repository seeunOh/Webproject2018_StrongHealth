...
<% 
	...
	String url = "Container.jsp?selected_page=music_read&article_number="+article.getArticle_number();

	if(session.getAttribute("authority") == null){
		out.println("<script>");
		out.println("alert('관리자만 음악글을 수정할 수 있습니다.')");
   ...
   <tr>
    ...
    <tr>
     ...
     <td colspan="2"  class="text-center">
      ...
      <input type="button" value="취소" class="btn btn-warning" onclick='move("Container.jsp?selected_page=music_read&article_number=<%=request.getParameter("article_number") %>");'>
     </td>
    </tr>
</table>
</form>
<script>
	document.getElementById('article_name').value = '<%=article.getArticle_name() %>';
 	document.getElementById('link').value = '<%=article.getLink() %>';
</script>
